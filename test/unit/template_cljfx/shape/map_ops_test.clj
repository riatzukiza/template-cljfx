(ns template-cljfx.shape.map-ops-test
  "Unit tests for map morphisms."
  (:require
   [clojure.test :refer [deftest is testing]]
   [template-cljfx.shape.map-ops :as sut]))
...
(deftest select-rename-test
  (testing "renames selected keys"
    (is (= {:x 1} (sut/select-rename {:a 1 :b 2} {:a :x}))))
  (testing "omits keys absent from source"
